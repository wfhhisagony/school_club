...
$.tale.prototype.alertConfirm2 = function (options) {
    options = options || {};
    options.title = options.title || '确定要报名吗？';
    options.text = options.text;
    options.showCancelButton = true;
    options.type = 'question';
    this.alertBox(options);
...
$.tale.prototype.alertBox2 = function (options) {
    swal({
        title: options.title,
        text: options.text,
        type: options.type,
        ...
        confirmButtonText: options.confirmButtonText || '报名',
        cancelButtonText: options.cancelButtonText || '取消'
    }).then(function (e) {
        options.then && options.then(e);
    }).catch(swal.noop);
